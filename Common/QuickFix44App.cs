...
namespace Common
{
    public sealed class QuickFix44App : MessageCracker, IApplication, IDisposable
    {
        ...
        public QuickFix44App(string username, string password, string senderCompId, string senderSubId, string targetCompId)
        {
            _username = username;
            _password = password;
        ...
        {
            Console.WriteLine($"{sessionID} - Logged in successfully");
            Logon?.Invoke();
        }

        public void OnLogout(SessionID sessionID)
        {
            Console.WriteLine($"{sessionID} - Logged out successfully");
            Logout?.Invoke();
        }
        ...
        public async void OnMessage(QuickFix.FIX44.Logout message, SessionID sessionID)
        {
            await _incomingMessagesBuffer.SendAsync(message);
        }
        public async void OnMessage(QuickFix.FIX44.Logon message, SessionID sessionID)
        {
            await _incomingMessagesBuffer.SendAsync(message);
        }
